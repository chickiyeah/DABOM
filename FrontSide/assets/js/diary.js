...
async function searchfood() {
  ...
  .then((data) => {
    ...
    data.json().then((items) => { // function a () {} <==> () => {} 화살표 배워올게요
      ...
      if (item_var != null) {
        let item_list = item_var;
        let item_data = item_list[0];
        console.log(item_data)
          console.log(item_var.식품명)// 아웃겨 답답하셨나봄 ㅋㅋ 네
          if(item_var != null){
            let html = `
            <li>
            <div class="checkbox">
                <input type="checkbox" id="check1">
                <label for="check1">체크하기</label>
            </div>
            <p class="info_txt">${item_var.식품명}</p>
            <div class="right_box">
                <p class="kcal">${item_var.칼로리} kcal</p>
                <a class="more_btn" href="javascript:">상세보기</a>
            </div>
        </li>
            `
